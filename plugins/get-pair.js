...
cmd({
    ...
    try {
        ...
        // Validate phone number format
        if (!phoneNumber || !phoneNumber.match(/^\+?\d{10,15}$/)) {
            return await reply("❌ Please provide a valid phone number with country code\nExample: .pair 2349079XXX");
        }

        // Make API request to get pairing code
        const response = await axios.get(`https://zuko-md-session-id.onrender.com/pair?phone=${encodeURIComponent(phoneNumber)}`);
